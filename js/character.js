...
async function init() {
  ...
  try {
    // data/{id}.json を直接取得（分割管理方式）
    const char = await fetch(`data/${id}.json`).then(r => {
      if (!r.ok) throw new Error(`data/${id}.json の取得失敗 (HTTP ${r.status})`);
      return r.json();
    });
  ...
  return `
    <div class="stats-section">
      ...
      <div class="stats-grid">
        <div class="stat-cell">
          <div class="stat-cell__label">基礎HP</div>
          <div class="stat-cell__value">${fmt(s.baseHP)}</div>
        </div>
        <div class="stat-cell">
          <div class="stat-cell__label">基礎攻撃力</div>
          <div class="stat-cell__value">${fmt(s.baseATK)}</div>
        </div>
        <div class="stat-cell">
          <div class="stat-cell__label">基礎防御力</div>
        ...
        <div class="stat-cell stat-cell--special">
          <div class="stat-cell__label">基礎ステータス</div>
...
function buildMultiplierTable(talent) {
  ...
  const rows = talent.multipliers.map(m => {
    ...
    const cells = LEVELS.map(lv => {
      ...
      const val = m.values[idx] ?? null;  // null → "—" で表示
      return `<td data-lv="${lv}" class="${lv === 1 ? 'is-active-level' : ''}" id="td_${talent.id}_${sanitize(m.label)}_${lv}">${fmtMult(val, usePercent)}</td>`;
    }).join('');
...
/**
 * 倍率をフォーマットする
 * @param {number|null} v - 値（null なら "—" を返す）
 * @param {boolean} usePercent - % を付けるか（デフォルト true）
 */
function fmtMult(v, usePercent = true) {
